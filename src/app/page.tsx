...
export default function Home() {
  ...
  function onFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    const file = event.target?.files?.[0]; // Use optional chaining to avoid errors if files is null
      ...
      .promise.then((pdf: any) => {
        ...
        pdf.getPage(1).then((page: any) => {
          page.getTextContent().then((content: any) => {
            ...
            console.log(text);
            sendToApi(text); // Call sendToApi with the extracted text
          });
        });
      });
  }
    ...
    <>
      ...
      >
        <div className="top-10 left-10 absolute flex items-center gap-4">
          ...
          <span className="text-2xl"> Wboursiquot </span>
        </div>
        <h1>Upload PDF</h1>
        <input
          className="hidden"
